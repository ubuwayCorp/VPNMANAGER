...
# Веб-сервер для предотвращения сна
app = Flask(__name__)

@app.route('/')
def home():
    return "✅ VPN Бот работает!"
...
def self_ping():
    time.sleep(60)  # Ждем минуту после запуска
    while True:
        try:
            requests.get("https://vpn-bot.onrender.com", timeout=5)
            print("✅ Пинг успешен")
        except:
            print("❌ Ошибка пинга")
        time.sleep(600)  # 10 минут

# Запускаем веб-сервер и пинг
keep_alive()
...
async def main():
    print("✅ Бот VPN Менеджер запущен на RENDER!")
    print(f"🤖 Bot: @VPNManagerRUbot")
    print("🌐 Веб-сервер для пинга активен")
